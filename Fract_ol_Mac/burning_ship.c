...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/03/01 16:21:56 by yhwang            #+#    #+#             */
/*   Updated: 2022/03/01 18:46:48 by yhwang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	burning_ship_set(double c_re, double c_im, t_mlx *mlx)
{
	...
	while (iter < ITER_MAX)
	{
		temp = x;
		x = x * x - y * y - c_re;
		y = fabs(2 * temp * y) - c_im;
		if (x * x + y * y > 4)
		{
			my_mlx_pixel_put(mlx, mlx->count_x, mlx->count_y,
				(mlx->color) + 0x00110000 * iter);
			return (0);
		}
		iter++;
	}
	...
	while (mlx->count_y < WIN_Y)
	{
		while (mlx->count_x < WIN_X)
		{
			...
			c_im = mlx->ymax - (mlx->count_y * (mlx->ymax - mlx->ymin)
					/ WIN_Y);
			burning_ship_set(c_re, c_im, mlx);
			mlx->count_x++;
		}
		mlx->count_y++;
		mlx->count_x = 0;
...
void	draw_burning_ship(void)
{
	...
	mlx.ymin = MINY;
	mlx.ymax = MAXY;
	if (!window_init_burning_ship(&mlx))
		return ;
	mlx.color = 0;
	mlx.fractol = 3;
	burning_ship((void *)&mlx);
